<template>
    <Teleport to=".teleport-modal">
        <div class="modal fade show" data-backdrop="static" id="addUnit" tabindex="-1" role="dialog"
...
<script setup>
import { ref } from 'vue';
import postUnit from '../../../../composables/unit/postUnit'
import $ from 'jquery'

const { addUnit } = postUnit()
const emit = defineEmits({ closeModal: false })
const name = ref('')

const handleSubmit = () => {

    addUnit({name: name.value}).then(() => {
        emit('closeModal')
        $('#addUnit').modal('hide')
    }).catch()
}
